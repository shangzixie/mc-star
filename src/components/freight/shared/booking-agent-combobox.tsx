...
export function BookingAgentCombobox({
  value,
  onValueChange,
  disabled = false,
  className,
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-[400px] p-0" align="start">
          <Command shouldFilter={false}>
            <CommandInput
              placeholder={t('searchBookingAgent')}
              value={searchQuery}
      ...
      <AddAgentDialog
        ...
        title={t('bookingAgentFields.addNew')}
        description={t('bookingAgentFields.addDescription')}
        submitLabel={t('bookingAgentFields.createSuccess')}
      />
          ...
          <Tooltip>
            ...
            <TooltipContent side="right" className="max-w-sm">
              <div className="space-y-2 text-sm">
                <div>
                  <strong>{t('bookingAgentFields.name')}:</strong>{' '}
                  {selectedAgent.name}
                </div>
                {selectedAgent.code && (
                  <div>
                    <strong>{t('bookingAgentFields.code')}:</strong>{' '}
                    {selectedAgent.code}
                  </div>
                )}
                {selectedAgent.address && (
                  <div>
                    <strong>{t('bookingAgentFields.address')}:</strong>{' '}
                    {selectedAgent.address}
                  </div>
                )}
                {!!selectedAgent.contactInfo &&
                  ...
                  selectedAgent.contactInfo !== null && (
                    <div>
                      <strong>{t('bookingAgentFields.contact')}:</strong>
                      {(() => {
